...
static int ag71xx_mac_pcs_get_state(struct phylink_config *config,
				     struct phylink_link_state *state)
{
	state->link = 0;
	return 1;
			    ...
			    struct phy_device *phy)
{
	...
	/* get this parameters via dirty phylink.c hack.
		 from 5.7.x kernel we do not need this. */
	struct phylink_link_state *link_state = config->__link_state;
	int speed = link_state->speed;
	int duplex = link_state->duplex;
	bool tx_pause = !!(link_state->pause & MLO_PAUSE_TX);
	bool rx_pause = !!(link_state->pause & MLO_PAUSE_RX);
...
static const struct phylink_mac_ops ag71xx_phylink_mac_ops = {
	...
	.mac_link_state = ag71xx_mac_pcs_get_state,
